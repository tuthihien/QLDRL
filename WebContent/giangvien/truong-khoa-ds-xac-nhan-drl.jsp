	...
	<div class="row">
		...
		<div class="col-sm-10">
			...
			<%} else { %>
				...
				<form action="<%=request.getContextPath() %>/giang-vien/truon
				g-khoa/danh-sach-xac-nhan" method="post">
				<div class="row">
					...
					<tbody>
						...
						<tr>
							...
							<td width="12%"><%=listSinhVien.get(i).getMssv() %></td>
								...
								<td width="15%">
									<%if (listDanhGia.get(i).getTinhTrang().equals("truongkhoadaduyet")) {%>
										<span class="glyphicon glyphicon-ok" aria-hidden="true"></span>
										<a href="<%=request.getContextPath() %>/giang-vien/xem-chi-tiet-danh-gia?id=<%=listDanhGia.get(i).getMaDanhGia() %>">Xem</a>
									<%} else if (listDanhGia.get(i).getTinhTrang().equals("gvcndaduyet")) { %>
					...
					<div class="col-sm-8" style="pad: 0px;">
						<ul class="pagination" style="margin: 0px;">
							<c:if test="${currentPage != 1}">
			        			<li><a href="<%=request.getContextPath() %>/giang-vien/truong-khoa/danh-sach-xac-nhan?lop=${maLopHienTai}&page=${currentPage - 1}">Previous</a></li>
			    			</c:if>
		 
		            		<c:forEach begin="1" end="${noOfPages}" var="i">
								<c:choose>
		                    		<c:when test="${currentPage eq i}">
		                        		<li class="active"><a href="<%=request.getContextPath() %>/giang-vien/truong-khoa/danh-sach-xac-nhan?lop=${maLopHienTai}&page=${i}">${i}</a></li>
		                   			 </c:when>
		                    		<c:otherwise>
		                        		<li><a href="<%=request.getContextPath() %>/giang-vien/truong-khoa/danh-sach-xac-nhan?lop=${maLopHienTai}&page=${i}">${i}</a></li>
		                    		</c:otherwise>
		                		</c:choose>
		            		</c:forEach>
		        			
							<c:if test="${currentPage lt noOfPages}">
		        				<li><a href="<%=request.getContextPath() %>/giang-vien/truong-khoa/danh-sach-xac-nhan?lop=${maLopHienTai}&page=${currentPage + 1}">Next</a></li>
		    				</c:if>
		    			</ul>
					</div>
				</div>
