		...
		<div class="col-sm-10">
			<div id="ketQuaDanhGia">
				...
				<div class="row" style="margin-top: 10px; ">
					...
					<form role="form" action="<%=request.getContextPath()%>/giang-vien/gvcn/du-lieu" method="post">
						...
						<a href="<%=request.getContextPath() %>/giang-vien/gvcn/in-an?dot=<%=dotDanhGia.getMaDotDanhGia() %>&lop=<%=lop.getMaLop() %>" 
							class="btn btn-success">In ấn</a>
					</form>
				</div>
				<br>
				...
				<ul class="pagination">
					<c:if test="${currentPage != 1}">
	        			<li><a href="<%=request.getContextPath() %>/giang-vien/gvcn/du-lieu?dot=<%=dotDanhGia.getMaDotDanhGia() %>&lop=<%=lop.getMaLop() %>&page=${currentPage - 1}">Previous</a></li>
	    			</c:if>
 
            		<c:forEach begin="1" end="${noOfPages}" var="i">
						<c:choose>
                    		<c:when test="${currentPage eq i}">
                        		<li class="active"><a href="<%=request.getContextPath() %>/giang-vien/gvcn/du-lieu?dot=<%=dotDanhGia.getMaDotDanhGia() %>&lop=<%=lop.getMaLop() %>&page=${i}">${i}</a></li>
                   			 </c:when>
                    		<c:otherwise>
                        		<li><a href="<%=request.getContextPath() %>/giang-vien/gvcn/du-lieu?dot=<%=dotDanhGia.getMaDotDanhGia() %>&lop=<%=lop.getMaLop() %>&page=${i}">${i}</a></li>
                    		</c:otherwise>
                		</c:choose>
            		</c:forEach>
        			
					<c:if test="${currentPage lt noOfPages}">
        				<li><a href="<%=request.getContextPath() %>/giang-vien/gvcn/du-lieu?dot=<%=dotDanhGia.getMaDotDanhGia() %>&lop=<%=lop.getMaLop() %>&page=${currentPage + 1}">Next</a></li>
    				</c:if>
    			</ul>
